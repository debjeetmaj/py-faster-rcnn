...
#coco image database with only 20 categories of pascal_voc
class minicoco(imdb):
    def __init__(self, image_set, year):
        imdb.__init__(self, 'minicoco_' + year + '_' + image_set)
        ...
        cats = self._COCO.loadCats(self._COCO.getCatIds(catNms=list(self.classes)[1:]))
        # self._class_to_coco_cat_id = dict(zip([c['name'] for c in cats],
        #                                       self._COCO.getCatIds()))
        self._class_to_coco_cat_id = {c['name']:c['id']  for c in cats if c['name']}
    ...
    def _load_image_set_index(self):
        ...
        Load image ids.
        """
        cocoCatIds = self._COCO.getCatIds(catNms=list(self.classes)[1:])
        s = Set([])
        for i in cocoCatIds :
    ...
    def target_roidb(self):
        """
        Return the database of images with empty gt boxes.
        This function loads/saves from/to a cache file to speed up future calls.
        """
        cache_file = osp.join(self.cache_path, self.name + '_target_roidb.pkl')
        if osp.exists(cache_file):
            with open(cache_file, 'rb') as fid:
                roidb = cPickle.load(fid)
            print '{} target roidb loaded from {}'.format(self.name, cache_file)
            return roidb

        target_roidb = [self._load_empty_coco_annotation(index)
                    for index in self._image_index]

        with open(cache_file, 'wb') as fid:
            cPickle.dump(target_roidb, fid, cPickle.HIGHEST_PROTOCOL)
        print 'wrote target roidb to {}'.format(cache_file)
        return target_roidb

    def _load_empty_coco_annotation(self,index):
        """
        loads empty COCO bounding-box instance annotations for each image
        """
        num_objs = 0
    ...
    def _load_coco_annotation(self, index):
        ...
        for ix, obj in enumerate(objs):
            #reject any category which is not part of the req 20 classes
            if not coco_cat_id_to_class_ind.has_key(obj['category_id']):
                continue
    ...
    def _do_detection_eval(self, res_file, output_dir):
        ...
        #set the required catIds and imgIds
        coco_eval.params.useCats = 1
        coco_eval.params.catIds = self._COCO.getCatIds(catNms=list(self.classes)[1:])
